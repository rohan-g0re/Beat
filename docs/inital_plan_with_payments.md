...
-- User entitlements (mirrors RevenueCat for RLS enforcement)
...
-- user_entitlements: owner read/update only (webhooks use service_role)
create policy "read own entitlements" on public.user_entitlements for select using (user_id = auth.uid());
create policy "update own entitlements" on public.user_entitlements for update using (user_id = auth.uid());

-- routines: owner-only with capacity check on insert
create policy "routines read update delete" on public.routines for select, update, delete
  using (user_id = auth.uid());

create policy "routines insert with cap" on public.routines for insert
  with check (
    user_id = auth.uid() 
    and (
      ...
      exists (
        ...
        and (e.pro = true or (select count(*) from public.routines where user_id = auth.uid()) < e.max_routines)
      )
...
* **Monetization**: `react-native-purchases` (RevenueCat SDK) for entitlement checking; Stripe Checkout for web payments.
